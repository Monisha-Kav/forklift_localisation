...
class TurtleBot3EKFLocalizer(Node):
    def __init__(self):
        ...
        self.R_lidar = np.diag([0.1, 0.1, 0.02])
    ...
    def lidar_callback(self, msg):
        angle = 0
        front_range = msg.ranges[angle]
        if not np.isnan(front_range) and front_range > 0.1:
            est_x = 1.0 - front_range
            z = np.array([est_x, self.state[1], self.state[2]])
            H = np.eye(3)
            y = z - self.state
            ...
            self.get_logger().info(f"[LIDAR] Corrected using LIDAR: x={self.state[0]:.2f}")
