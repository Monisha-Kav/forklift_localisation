...
CRATE_POS = [0.15, 1.7]  # hardcoded crate position
...
class ForkliftDisplay(Node):
    ...
    def robot_callback(self, msg):
        ...
        if self.goal_pos and self.forklift_pos:
            ...
            if dist < 0.1 and self.state in ["TO_CRATE", "TO_GOAL"]:
    ...
    def draw_display(self):
        ...
        if self.state == "TO_CRATE" and self.robot_pos.distance_to(CRATE_POS) < 5:
            self.state = "IDLE"
            self.goal_pos = None
        ...
        elif self.state == "TO_GOAL" and self.goal_pos and self.robot_pos.distance_to(self.goal_pos) < 15:
            self.state = "IDLE"
            self.goal_pos = None
